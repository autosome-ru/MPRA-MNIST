"""
AgarwalMulti_model_launch.py

Runs a benchmark: trains a single multi-output model (one output per cell
type) on the Agarwal MPRA dataset, repeating the whole process `--runs` times
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        "cell type) on the Agarwal MPRA dataset.",
...
def build_transforms(dataset_cls: type[AgarwalMultiDataset], sequence_first: bool = False) -> dict:
...
def build_model(model_name: str, in_channels: int, seq_len: int, num_outputs: int):
    ...
    if model_name == "MPRALegNet":
        ...
        model = DREAM_RNN(in_channels, 230, num_outputs)
...
def evaluate_with_reverse_complement(
    ...
    trainer: L.Trainer,
    lit_model: LitModel_AgarwalMulti,
    cell_types: list[str],
    num_outputs: int,
) -> torch.Tensor:
...
def run_single_training_run(run_idx: int, args: argparse.Namespace, transforms: dict) -> torch.Tensor:
    ...
    lit_model = LitModel_AgarwalMulti.load_from_checkpoint(
        best_model_path,
        model=model,
        loss=loss,
        weight_decay=args.wd,
    ...
    test_forward_dataset = AgarwalMultiDataset(
        cell_type=args.cell_types, split="test", transform=transforms["forward"], root=args.root
    )
    test_reverse_dataset = AgarwalMultiDataset(
...
def main() -> None:
    ...
    transforms = build_transforms(AgarwalMultiDataset, sequence_first=sequence_first)
