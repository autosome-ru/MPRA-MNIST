...
dataset_args.add_argument("--cell_types_variant",
                     nargs='+',            # accepts one or more values
                     default=['AST', 'ES', 'N-D2', 'N-D4', 'N-D10', 'A-NPC', 'D283', 'D341', 'IMR.diff', 'IMR.prog', 'SHSY5Y.diff', 'SHSY5Y.prog', 'HEK293T'],
...
for run in list(range(args.runs)):
    ...
    seq_model = LitModel_Guo.load_from_checkpoint(best_model_path,model=model, loss=nn.MSELoss(), weight_decay=args.wd, lr=args.lr,  print_each=1, use_one_cycle=use_one_cycle)

    predict_forward_dataset = GuoMultiDataset(split = 'test', length = 145, cell_types = args.cell_types_variant, transform=forw_transform, root =args.root)
    ...
    predict_revcomp_dataset = GuoMultiDataset(split = 'test', length = 145, cell_types = args.cell_types_variant, transform=revcomp_transform, root =args.root)
    ...
    results = get_variant_predictions(forw_preds, revcomp_preds, args.cell_types_variant)
