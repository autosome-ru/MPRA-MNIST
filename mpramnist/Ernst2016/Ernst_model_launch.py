"""
Ernst_model_launch.py

Runs a benchmark: trains a multi-output model (one output per cell type) on
the Ernst2016 dataset, then fine-tunes it for a few epochs at a reduced
learning rate/weight decay, repeating the whole process `--runs` times.
Test-time evaluation averages forward-strand and reverse-complement
predictions before computing the Pearson correlation per cell type.

Results (one row per run, one column per cell type) are appended to a TSV
...
from mpramnist.Ernst2016 import ErnstDataset, LitModel_Ernst
...
FINETUNE_WD_FACTOR = 0.01


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a multi-output model (one output per "
        "cell type) on the Ernst2016 dataset, with a fine-tuning stage.",
    ...
    dataset_args.add_argument(
        "--cell_types",
        nargs="+",
        default=["k562_minp_avg", "k562_sv40p_avg", "hepg2_minp_avg", "hepg2_sv40p_avg"],
    ...
    trainer_args.add_argument("--finetune_epoch_num", type=int, default=5, help="Finetune number of epochs.")
...
def build_transforms(dataset_cls: type[ErnstDataset], sequence_first: bool = False) -> dict:
...
# =============================================================================
# Single train + fine-tune + eval run (all cell types trained jointly)
# =============================================================================

def run_single_training_run(run_idx: int, args: argparse.Namespace, transforms: dict) -> torch.Tensor:
    """Train a fresh multi-output model on all requested cell types, fine-tune
    it for a few epochs at a reduced learning rate/weight decay, and return
    the per-cell-type test Pearson correlations.
    """
    ...
    val_dataset = ErnstDataset(
        cell_type=args.cell_types, split="val", transform=transforms["test"], root=args.root
    )
    ...
    # ---- Trainer setup --------------------------------------------------
    logger = pl_loggers.TensorBoardLogger(f"./{args.model}_logs", name="Ernst")
    ...
    # ---- Fine-tune: reload best checkpoint, then train a few more epochs
    #      at a reduced learning rate / weight decay -----------------------
    best_model_path = checkpoint_callback.best_model_path
    lit_model_finetune = LitModel_Ernst.load_from_checkpoint(
        best_model_path,
        model=model,
        loss=loss,
        weight_decay=args.wd * FINETUNE_WD_FACTOR,
        lr=args.lr * FINETUNE_LR_FACTOR,
        cell_types=args.cell_types,
        print_each=1,
        learning_rate_reduce=learning_rate_reduce,
    )
    ...
    trainer = L.Trainer(
        accelerator="gpu",
        devices=[args.device],
        max_epochs=args.finetune_epoch_num,
        gradient_clip_val=1,
        precision="16-mixed",
    ...
    trainer.fit(lit_model_finetune, train_dataloaders=train_loader, val_dataloaders=val_loader)
    ...
    test_forward_dataset = ErnstDataset(
        cell_type=args.cell_types, split="test", transform=transforms["forward"], root=args.root
    )
    test_reverse_dataset = ErnstDataset(
        cell_type=args.cell_types, split="test", transform=transforms["reverse"], root=args.root
    )
...
def main() -> None:
    ...
    transforms = build_transforms(ErnstDataset, sequence_first=sequence_first)
