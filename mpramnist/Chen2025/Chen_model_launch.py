...
effects in the Chen2025 dataset: forward-strand and reverse-complement
predictions are averaged, the ref/alt effect size is computed and oriented by
`reverse_prediction`, and its Pearson correlation with the targets is
reported per cell type/state (both overall and restricted to significant
daSNPs, fdr < 0.05).
...
GOSAI_CROP_LEN = 600
CHEN_VARIANT_LEN = 227

# Full list of cell-type/state columns predicted on the Chen2025 variant set.
FULL_CELL_TYPES = [
    "THP1_aggregated", "THP1_Naive", "THP1_IFNB", "THP1_IFNG", "THP1_LPSIFNG",
    "HMC3_aggregated", "HMC3_Naive", "HMC3_IFNB", "HMC3_IFNG", "HMC3_LPSIFNG",
    "Brain_aggregated", "Brain_Cortex", "Brain_Hippocampus", "Brain_Striatum",
]

# Grouping reference for FULL_CELL_TYPES (kept for reference, not consumed elsewhere).
CELL_TYPE = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a multi-output model on Gosai2024 and evaluate variant effects on Chen2025.",
    ...
    dataset_args.add_argument(
        ...
        nargs="+",
        default=None,
        help="List of cell types for the Chen2025 variant evaluation.",
    )
    dataset_args.add_argument(
        "--states_variant",
        nargs="+",
...
def build_transforms(dataset_cls: type[GosaiDataset], sequence_first: bool = False) -> dict:
    ...
    applied to *Chen* variant sequences, embedding them in the same context
    ...
    # Used at test time (on ChenMultiDataset) to average forward-strand and
...
def build_model(model_name: str, in_channels: int, seq_len: int, num_outputs: int):
    ...
    Returns (model, loss, use_one_cycle).
    """
    loss = nn.MSELoss()
...
def evaluate_variant_effects(
    ...
    orient the effect size by `reverse_prediction`, and compute the Pearson
    correlation (per cell type/state) against the targets — both over all
    variants and restricted to significant daSNPs (fdr < 0.05).
    """
    targets = torch.cat([batch["target"] for batch in forward_preds])
    fdrs = torch.cat([batch["fdr"] for batch in forward_preds])
    reverse_prediction = torch.cat([batch["reverse_prediction"] for batch in forward_preds])
    ...
    variant_prediction = (mean_alt - mean_ref).squeeze() * reverse_prediction.squeeze()

    pearson_metric = PearsonCorrCoef()
    results = []

    for i in range(len(cell_types)):
        pearsonr_all = pearson_metric(variant_prediction.squeeze(), targets[:, i].squeeze())

        significant_mask = fdrs[:, i].squeeze() < 0.05  # keep only significant daSNPs
...
def run_single_training_run(run_idx: int, args: argparse.Namespace, transforms: dict) -> pd.DataFrame:
    ...
    per-cell-type/state variant-effect Pearson correlations on Chen2025.
    """
    ...
    lit_model = LitModel_Chen.load_from_checkpoint(
        best_model_path,
        model=model,
        loss=nn.MSELoss(),
        weight_decay=args.wd,
    ...
    # ---- Test: evaluate variant effects on Chen2025, averaging
    #      forward-strand and reverse-complement predictions ---------------
    predict_forward_dataset = ChenMultiDataset(
        split="test",
        length=CHEN_VARIANT_LEN,
        cell_types=args.cell_types_variant,
        states=args.states_variant,
        interval_type=args.interval_type,
        transform=transforms["forward"],
        root=args.root,
    )
    predict_reverse_dataset = ChenMultiDataset(
        split="test",
        length=CHEN_VARIANT_LEN,
        cell_types=args.cell_types_variant,
        states=args.states_variant,
    ...
    results = evaluate_variant_effects(forward_preds, reverse_preds, FULL_CELL_TYPES)
