"""
Wang_model_launch.py

Runs a benchmark: for a requested model architecture, trains and evaluates it
on the Wang MPRA dataset, repeating the whole process `--runs` times to get a
distribution of Pearson correlations.

Results (one row per run) are appended to a TSV file given by `--result_dir`.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a model on the Wang MPRA dataset.",
    ...
    trainer_args.add_argument("--wd", type=float, default=1e-4, help="Weight decay (AdamW).")
    trainer_args.add_argument("--batch_size", type=int, default=128, help="Batch size.")
...
def build_model(model_name: str, in_channels: int, seq_len: int):
    ...
    Returns (model, loss). Scheduler-related flags are omitted for Wang.
    """
    loss = nn.MSELoss()
...
def evaluate_with_reverse_complement(
    ...
    print("===========")
    print(f"Wang Pearson correlation: {pearson}")
    print("===========")
...
def run_single_run(args: argparse.Namespace, transforms: dict) -> torch.Tensor:
    """Train a fresh model on the Wang dataset and return its test Pearson correlation."""

    # ---- Data -------------------------------------------------------------
    train_dataset = WangDataset(
        split="train", transform=transforms["train"], root=args.root
    )
    val_dataset = WangDataset(
        split="val", transform=transforms["test"], root=args.root
    )
    ...
    # ---- Trainer setup ----------------------------------------------------
    logger = pl_loggers.TensorBoardLogger(f"./{args.model}_logs", name="Wang")
    ...
    lit_model = LitModel_Wang.load_from_checkpoint(
        best_model_path,
        model=model,
        loss=loss,
        weight_decay=args.wd,
    ...
    test_forward_dataset = WangDataset(
        split="test", transform=transforms["forward"], root=args.root
    )
    test_reverse_dataset = WangDataset(
        split="test", transform=transforms["reverse"], root=args.root
    )
    ...
    pearson = evaluate_with_reverse_complement(
        forward_loader, reverse_loader, trainer, lit_model, num_outputs=1
    )
...
def load_or_create_results(result_path: str) -> pd.DataFrame:
    if os.path.exists(result_path):
        return pd.read_csv(result_path, sep="\t")
    return pd.DataFrame(columns=["Corr_pearson"])


def main() -> None:
    args = parse_args()
    results = load_or_create_results(args.result_dir)
    ...
    for run in range(args.runs):
        ...
        pearson = run_single_run(args, transforms)
