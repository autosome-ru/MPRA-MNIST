...
for run in list(range(args.runs)):
    ...
    train_dataset = ReddyDataset(task=task,cell_type=args.cell_types, split="train", transform=train_transform, root=args.root,)
    val_dataset = ReddyDataset(task=task,cell_type=args.cell_types, split="val", transform=val_test_transform, root=args.root,)
    test_dataset = ReddyDataset(task=task,cell_type=args.cell_types, split="test", transform=val_test_transform, root=args.root,)
    ...
    seq_model = LitModel_Reddy_Reg(model=model, loss=nn.MSELoss(), weight_decay=args.wd, lr=args.lr, cell_types=args.cell_types, print_each=1)
    ...
    seq_model = LitModel_Reddy_Reg.load_from_checkpoint(best_model_path,model=model, loss=nn.MSELoss(), weight_decay=args.wd, lr=args.lr, cell_types=args.cell_types, print_each=1)

    forw_transform = t.Compose([t.Seq2Tensor()])
    rev_transform = t.Compose([t.ReverseComplement(1),t.Seq2Tensor()])

    test_forw = ReddyDataset(task=task, cell_type=args.cell_types, split="test", transform=forw_transform, root=args.root)
    test_rev = ReddyDataset(task=task, cell_type=args.cell_types, split="test", transform=rev_transform, root=args.root)
